...
LOCAL_DEV_BYPASS_TOKEN = "__local_dev_bypass__"


def _is_local_dev_auth_bypass_enabled() -> bool:
    return os.getenv("LOCAL_DEV_AUTH_BYPASS", "").lower() in {"1", "true", "yes"}
...
async def get_bearer_token(
    ...
    if _is_local_dev_auth_bypass_enabled():
        logger.warning("LOCAL_DEV_AUTH_BYPASS is enabled for %s %s", request.method, request.url.path)
        return LOCAL_DEV_BYPASS_TOKEN
...
async def get_current_user(token: str = Depends(get_bearer_token)) -> UserResponse:
    ...
    if token == LOCAL_DEV_BYPASS_TOKEN:
        return UserResponse(
            id=os.getenv("LOCAL_DEV_USER_ID", "local-dev-user"),
            email=os.getenv("LOCAL_DEV_USER_EMAIL", "local-dev@example.com"),
            name=os.getenv("LOCAL_DEV_USER_NAME", "Local Dev User"),
            role=os.getenv("LOCAL_DEV_USER_ROLE", "admin"),
