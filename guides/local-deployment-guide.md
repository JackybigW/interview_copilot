...
- **简历/JD 结构化分析**：使用 GLM-5（通灵义 DashScope API）+ LangChain 进行结构化信息提取
...
│   │   └── settings.py      # 设置管理
│   ├── services/            # 业务逻辑层
│   │   ├── glm_structured_service.py   # GLM-5 结构化提取
...
│
└── guides/local-deployment-guide.md  # 本文件
```
...
| 环境变量 | 服务 | 用途 | 获取地址 |
|---------|------|------|---------|
| `DASHSCOPE_API_KEY` | 阿里云 DashScope | GLM-5 结构化分析（简历/JD提取） | https://dashscope.console.aliyun.com/ |
| `DASHSCOPE_BASE_URL` | 阿里云 DashScope | API 端点 | 默认: `https://dashscope.aliyuncs.com/compatible-mode/v1` |
...
> - ✅ 正确: `DASHSCOPE_API_KEY=sk-abc123`
> - ❌ 错误: `DASHSCOPE_API_KEY="sk-abc123"` 或 `DASHSCOPE_API_KEY = sk-abc123`
...
- `alembic` — 数据库迁移
- `langchain-openai` + `langchain-core` — GLM-5 结构化输出
...
| 方法 | 路径 | 说明 |
|------|------|------|
| POST | `/api/v1/interview/analyze-structured` | GLM-5 结构化分析（简历/JD） |
...
│                    Workspace 页面                         │
│                                                          │
│  1. 上传/粘贴简历 ──→ GLM-5 结构化提取 ──→ ResumeProfile │
│  2. 粘贴 JD ──────→ GLM-5 结构化提取 ──→ JDProfile       │
...
简历文本/文件 ──→ [后端] file_parser_service ──→ 纯文本
                          ↓
                  glm_structured_service (GLM-5 + LangChain)
                          ↓
...
curl -s https://dashscope.aliyuncs.com/compatible-mode/v1/chat/completions \
  ...
  -d '{"model":"glm-5","messages":[{"role":"user","content":"hello"}],"max_tokens":10}'
```
...
| **AI - 结构化分析** | GLM-5 via DashScope + LangChain | - |
...
- [ ] `DASHSCOPE_API_KEY` 已配置（GLM-5 分析必需）
